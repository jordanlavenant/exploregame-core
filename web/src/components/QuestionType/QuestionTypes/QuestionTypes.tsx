import type {
  DeleteQuestionTypeMutation,
  DeleteQuestionTypeMutationVariables,
  FindQuestionTypes,
} from 'types/graphql'

import { Link, routes } from '@redwoodjs/router'
import { useMutation } from '@redwoodjs/web'
import type { TypedDocumentNode } from '@redwoodjs/web'
import { toast } from "sonner"

import { QUERY } from 'src/components/QuestionType/QuestionTypesCell'
import { truncate } from 'src/lib/formatters'

const DELETE_QUESTION_TYPE_MUTATION: TypedDocumentNode<
  DeleteQuestionTypeMutation,
  DeleteQuestionTypeMutationVariables
> = gql`
  mutation DeleteQuestionTypeMutation($id: String!) {
    deleteQuestionType(id: $id) {
      id
    }
  }
`

const QuestionTypesList = ({ questionTypes }: FindQuestionTypes) => {
  const [deleteQuestionType] = useMutation(DELETE_QUESTION_TYPE_MUTATION, {
    onCompleted: () => {
      toast.success('QuestionType deleted')
    },
    onError: (error) => {
      toast.error(error.message)
    },
    // This refetches the query on the list page. Read more about other ways to
    // update the cache over here:
    // https://www.apollographql.com/docs/react/data/mutations/#making-all-other-cache-updates
    refetchQueries: [{ query: QUERY }],
    awaitRefetchQueries: true,
  })

  const onDeleteClick = (id: DeleteQuestionTypeMutationVariables['id']) => {
    if (confirm('Are you sure you want to delete questionType ' + id + '?')) {
      deleteQuestionType({ variables: { id } })
    }
  }

  return (
    <div className="rw-segment rw-table-wrapper-responsive">
      <table className="rw-table">
        <thead>
          <tr>
            <th>Id</th>
            <th>Type</th>
            <th>&nbsp;</th>
          </tr>
        </thead>
        <tbody>
          {questionTypes.map((questionType) => (
            <tr key={questionType.id}>
              <td>{truncate(questionType.id)}</td>
              <td>{truncate(questionType.type)}</td>
              <td>
                <nav className="rw-table-actions">
                  <Link
                    to={routes.questionType({ id: questionType.id })}
                    title={'Show questionType ' + questionType.id + ' detail'}
                    className="rw-button rw-button-small"
                  >
                    Show
                  </Link>
                  <Link
                    to={routes.editQuestionType({ id: questionType.id })}
                    title={'Edit questionType ' + questionType.id}
                    className="rw-button rw-button-small rw-button-blue"
                  >
                    Edit
                  </Link>
                  <button
                    type="button"
                    title={'Delete questionType ' + questionType.id}
                    className="rw-button rw-button-small rw-button-red"
                    onClick={() => onDeleteClick(questionType.id)}
                  >
                    Delete
                  </button>
                </nav>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  )
}

export default QuestionTypesList
